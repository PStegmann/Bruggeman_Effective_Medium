...
{
  debug = 0;
  /****	READ calcite refractive index database ****/

  std::ifstream source; 							// build a read-Stream
  source.open("./real/realpart_Calcite.txt", std::ios_base::in);	// open data
  if(!source)
  {
  ...
  else
  {
    ...
    if ( debug ) std::cout << "number of lines of the calcite database: " << counter << std::endl;
  }
  source.close();

  source.open("./imag/imagpart_Calcite.txt", std::ios_base::in);	// open data
  if(!source)
  {
    std::cerr << "Refractive index data file not found!\n";
  }
  ...
  if (debug ) std::cout << "3. calcite database read from file." << std::endl;
};

minerals::calcite::~calcite(){};

std::complex<double> minerals::calcite::operator()(double wavelength)
{
