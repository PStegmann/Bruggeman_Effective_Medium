...
{
  debug = 0;
  /****	READ water refractive index database ****/

  std::ifstream source; 							// build a read-Stream
  source.open("./refr_Water.txt", std::ios_base::in);	// open data
  if(!source)
  {
  ...
  else
  {
    ...
    for (std::string line; getline(source,line); ) // read stream line by line
    {
      if(counter > 0) // stream also reads bullshit lines at the beginning and end.
      {
        ...
        double x[3]; // input buffer for database
        in >> x[0];	// wavelength in mu
        in >> x[1];	// real part of refractive index
        in >> x[2]; // imaginary part
        if ( debug ) std::cout << x[0] << " " << x[1] << " ";	// output for debbuging
        database_l.push_back(x[0]);
        database_n.push_back(x[1]);
        database_lk.push_back(x[0]);
        database_k.push_back(x[2]);
      }
      counter += 1;
    }
    if ( debug ) std::cout << "number of lines of the water database: " << counter << std::endl;
  }
  source.close();

  if (debug) std::cout << "8. water database read from file." << std::endl;
};
...
std::complex<double> minerals::water::operator()(double wavelength)
{
