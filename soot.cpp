...
{
  debug = 0;
  /****	READ soot refractive index database ****/

  std::ifstream source; 							// build a read-Stream
  source.open("./real/realpart_Soot.txt", std::ios_base::in);	// open data
  if(!source)
  {
  ...
  else
  {
    ...
    if ( debug ) std::cout << "number of lines of the soot database: " << counter << std::endl;
  }
  source.close();

  source.open("./imag/imagpart_Soot.txt", std::ios_base::in);	// open data
  if(!source)
  {
    std::cerr << "Refractive index data file not found!\n";
  }
  ...
  if (debug) std::cout << "7. soot database read from file." << std::endl;
};

minerals::soot::~soot(){};

std::complex<double> minerals::soot::operator()(double wavelength)
{
