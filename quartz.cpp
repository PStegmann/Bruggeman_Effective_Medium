...
minerals::quartz::quartz()
{
  ...
  source.open("./real/realpart_Quartz.txt", std::ios_base::in);	// open data
  if(!source)
  {
  ...
  else
  {
    ...
    if ( debug ) std::cout << "number of lines of the quartz database: " << counter << std::endl;
  }
  source.close();

  source.open("./imag/imagpart_Quartz.txt", std::ios_base::in);	// open data
  if(!source)
  {
    std::cerr << "Refractive index data file not found!\n";
  }
  ...
  if (debug) std::cout << "2. Quartz database read from file." << std::endl;
};

minerals::quartz::~quartz(){};

std::complex<double> minerals::quartz::operator()(double wavelength)
{
