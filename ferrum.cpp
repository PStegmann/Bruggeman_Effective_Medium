...
minerals::ferrum::ferrum()
{
  debug = 0;
  /****	READ ferrum refractive index database ****/

  std::ifstream source; 							// build a read-Stream
  source.open("./real/realpart_Ferrum.txt", std::ios_base::in);	// open data
  if(!source)
  {
  ...
  else
  {
    ...
    if ( debug ) std::cout << "number of lines of the ferrum database: " << counter << std::endl;
  }
  source.close();

  source.open("./imag/imagpart_Ferrum.txt", std::ios_base::in);	// open data
  if(!source)
  {
    std::cerr << "Refractive index data file not found!\n";
  }
  ...
  if(debug) std::cout << "iron oxide database read from file." << std::endl;
};

minerals::ferrum::~ferrum(){};

std::complex<double> minerals::ferrum::operator()(double wavelength)
{
  ...
  if (wavelength < *(database_lk.begin()) ) // Extrapolation below available data
  {
    float temp=1.f;
