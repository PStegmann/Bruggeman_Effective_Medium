...
minerals::sulphates::sulphates()
{
  debug = 0;
  /****	READ Silicate refractive index database ****/

  std::ifstream source; 							// build a read-Stream
  source.open("./real/realpart_Sulphates.txt", std::ios_base::in);	// open data
  if(!source)
  {
  ...
  source.open("./imag/imagpart_Sulphates.txt", std::ios_base::in);	// open data
  if(!source)
  {
    std::cerr << "Refractive index data file not found!\n";
  }
  else
  {
    ...
    if ( debug ) std::cout << "number of lines of the sulphates database: " << counter << std::endl;
  }
  source.close();

  if ( debug) std::cout << "2. Sulphates database read from file." << std::endl;
};

minerals::sulphates::~sulphates(){};

std::complex<double> minerals::sulphates::operator()(double wavelength)
{
